...
2026-08-09 - feat(migraphx): library-level compile-hang fix shipped — env-gated patched-core build (`MLSTACK_MIGRAPHX_CORE_FIX=1` → `migraphx_python.rs::build_fixed_core_commands`, 6-step: vendor nlohmann-json, clone AMDMIGraphX `rocm-7.2.3`, `git apply` vendored PR #5106 backport `patches/amdmigraphx-5106-find_concat_transpose.patch`, configure with CK/MLIR off matching the distro package, build, install to `~/.mlstack/migraphx-fixed`). The patch (upstream commit `b90f58a7e63a`, merged 2026-07-31) rewrites `find_concat_transpose::apply` to read the permutation from the transpose op instead of `assert(s.transposed())` — the verified root cause of BOTH the `simplify_reshapes.cpp:845` crash and the `repeat_while_changes` non-convergence hang in MIGraphX 2.15.0. Same SONAME (`2015000`), so ORT's MIGraphX EP loads it unchanged; the persistent-env generator (env_setup.rs) now emits an idempotent guarded `LD_LIBRARY_PATH` prepend for `~/.mlstack/migraphx-fixed/lib/migraphx/lib` (bash + fish, `$HOME`-relative at source-time, mirroring the RCCL overlay block) — RUNPATH-after-LD_LIBRARY_PATH precedence means the patched lib wins without touching `/opt/rocm`. The native installer dispatch (`installer.rs` migraphx-python arm) runs the patched-core build when `MLSTACK_MIGRAPHX_CORE_FIX=1` (patch resolved from `CARGO_MANIFEST_DIR`; refreshes `~/.mlstack_env` afterward so the overlay is emitted). Build commands are idempotent (skip-if-present clone/apply guards, quoted paths). VERIFIED on this host: probe L1 (was TIMEOUT) + L2 (was CRASH at simplify_reshapes.cpp:845) now PASS against the patched core at batch 1 and 2. Also fixed the shipped model-optimizer command (`onnxruntime.rs::build_model_optimizer_command`) to run **CPU-only** pre-opt (probe L8 proof: an EP inside the pre-opt session bakes unserializable compiled nodes / can hang — the exact latent bug the earlier step shipped).
