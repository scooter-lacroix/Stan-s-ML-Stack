...
|---|---|---|
| `src/installers/common/package_manager.rs` | Package manager abstraction (apt/dnf/pacman) | `lib/package_manager.sh` |
| `src/installers/common/distro.rs` | Distro detection (delegates to `platform::detection`) | `lib/distro_detection.sh` |
| `src/installers/common/guard.rs` | Structured error handling and logging | `lib/installer_guard.sh` |
| `src/installers/common/rocm_env.rs` | ROCm environment configuration | `lib/rocm_env.sh` |
| `src/installers/common/utils.rs` | Shared utilities (color output, command checks) | `common_utils.sh` |
| `src/installers/common/env_validation.rs` | Environment file validation | `env_validation_utils.sh` |
| `src/installers/common/benchmark_common.rs` | Benchmark result parsing and logging | `lib/benchmark_common.sh` |
| `src/installers/common/ui_helper.rs` | CLI argument parsing and validation | `lib/ui_installer_helper.sh` |
| `src/installers/common/package_mappings.rs` | Package name mapping per distro | `lib/package_mappings.sh` |
| `src/installers/components/*.rs` | 24 native component installers | `scripts/install_*.sh` |
| `src/verification/mod.rs` | Installation verification | `scripts/verify_*.sh` |
| `src/benchmark_runners/mod.rs` | Benchmark execution | `scripts/run_*_benchmarks.sh` |
| `src/bootstrap/` | Install, launch, environment setup | `scripts/install.sh`, `run_rusty_stack.sh`, `enhanced_setup_environment.sh` |
...
# src/bootstrap/env_setup.rs — equivalent to enhanced_setup_environment.sh
# src/bootstrap/install.rs   — equivalent to install.sh
# src/bootstrap/launcher.rs  — equivalent to run_rusty_stack.sh
```
