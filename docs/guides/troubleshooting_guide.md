...
5. **Library-level fix** (the REAL fix — SHIPPED, env-gated): the defect is confirmed in `migraphx::version_2_15_0` itself (built from AMDMIGraphX `rocm-7.2.3`): `simplify_reshapes.cpp find_concat_transpose` deduces transposed-ness from strides and asserts `s.transposed()`, and the neighboring reshape passes never converge (`repeat_while_changes`). Upstream fixed it in PR #5106 "Fix find_concat_transpose with non-transposed inputs" (merged 2026-07-31, commit `b90f58a7e63a`) — reads the permutation from the transpose op instead of asserting. rusty-stack ships an env-gated patched-core build (`MLSTACK_MIGRAPHX_CORE_FIX=1` → `installers/components/migraphx_python.rs::build_fixed_core_commands`): builds AMDMIGraphX `rocm-7.2.3` + the vendored backport (`rusty-stack/patches/amdmigraphx-5106-find_concat_transpose.patch`), installs to `~/.mlstack/migraphx-fixed` (same SONAME `2015000`), and the persistent-env generator prepends its `lib` dir to `LD_LIBRARY_PATH` (the ORT provider uses RUNPATH, searched after `LD_LIBRARY_PATH`, so the patched lib wins without touching `/opt/rocm`). No root needed; `git apply` is pure C++ and compiles against the installed ROCm 7.2.4 toolchain with `MIGRAPHX_USE_COMPOSABLEKERNEL=Off` + `MIGRAPHX_ENABLE_MLIR=Off` (matching the distro package config).
