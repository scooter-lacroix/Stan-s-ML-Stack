...
```bash
# Primary installer — unified rusty-stack CLI (TUI mode)
cd rusty-stack && cargo build --release
./target/release/rusty-stack        # Launch interactive TUI installer
...
```bash
rusty-stack update                  # Scan and update components
rusty-stack update --scan-only      # Scan without applying changes
rusty-stack update --all-safe       # Apply all safe updates
rusty-stack update --json           # Machine-readable output
```

### Verification

```bash
rusty-stack verify --full           # Full component verification
rusty-stack verify --enhanced       # Enhanced verification (all components)
rusty-stack verify --build          # Verify and rebuild failed components
```

### Benchmarks

```bash
rusty-stack bench --all             # Run full benchmark suite
rusty-stack bench --rocm            # ROCm benchmarks
rusty-stack bench --json <name>     # JSON output for a specific benchmark
```

### Environment Setup

```bash
# Environment setup is handled by the rusty-stack CLI bootstrap module
...
│   ├── install_ml_stack_curses.py  # DEPRECATED — use rusty-stack CLI
│   └── install_ml_stack_ui.py      # DEPRECATED — use rusty-stack CLI
...
1. **Unified `rusty-stack` CLI (Primary):** Single binary with clap subcommands. `rusty-stack` (no args) launches TUI. Subcommands: `update` (scan/plan/apply/verify), `upgrade` (binary/runtime upgrade), `bench` (benchmark runner with --json), `verify` (installation verification with --full/--enhanced/--build).
2. **Rusty-Stack TUI:** Interactive terminal UI using ratatui + crossterm. Accessible via `rusty-stack` (no args). All 24 installer components use native Rust — no bash subprocess for installation.
...
7. **Python UIs (Deprecated):** Textual and curses-based installers marked deprecated. Use `rusty-stack` CLI instead.
