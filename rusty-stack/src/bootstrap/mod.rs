...
mod tests {
    ...
    fn test_install_config_defaults() {
        ...
        assert_eq!(config.repo_url, "https://github.com/scooter-lacroix/Stan-s-ML-Stack.git");
    ...
    fn test_install_result_success() {
        let result = InstallResult::success("/tmp/repo".to_string(), "/tmp/repo/rusty-stack/target/release/rusty-stack".to_string());
    ...
    fn test_system_dependency_list() {
        ...
        assert!(deps.contains(&"python3-dev"), "python3-dev should be required");
        assert!(deps.contains(&"build-essential"), "build-essential should be required");
    }
    ...
    fn test_env_file_content_has_required_variables() {
        ...
        assert!(content.contains("HIP_VISIBLE_DEVICES"), "env file must set HIP_VISIBLE_DEVICES");
        assert!(content.contains("CUDA_VISIBLE_DEVICES"), "env file must set CUDA_VISIBLE_DEVICES");
        assert!(content.contains("PYTORCH_ROCM_DEVICE"), "env file must set PYTORCH_ROCM_DEVICE");
        assert!(content.contains("ROCM_HOME"), "env file must set ROCM_HOME");
        assert!(content.contains("ROCM_VERSION"), "env file must set ROCM_VERSION");
        assert!(content.contains("ROCM_CHANNEL"), "env file must set ROCM_CHANNEL");
        assert!(content.contains("GPU_ARCH"), "env file must set GPU_ARCH");
        assert!(content.contains("LD_LIBRARY_PATH"), "env file must set LD_LIBRARY_PATH");
    ...
    fn test_env_file_content_is_shell_sourceable() {
        ...
        assert!(content.starts_with("# ML Stack Environment File"), "env file must have header comment");
        assert!(content.contains("export "), "env file must have export statements");
        // No bash-specific syntax errors: should not contain unmatched quotes
        let export_count = content.matches("export ").count();
        assert!(export_count >= 10, "should have at least 10 export statements, got {export_count}");
    }
    ...
    fn test_rocm_optimization_hsa_override() {
        ...
        assert!(content.contains("HSA_OVERRIDE_GFX_VERSION"), "must set HSA_OVERRIDE_GFX_VERSION");
        assert!(content.contains("11.0.0"), "must contain the correct HSA_OVERRIDE_GFX_VERSION value");
    }
    ...
    fn test_rocm_optimization_miopen_settings() {
        ...
        assert!(content.contains("MIOPEN_DEBUG_CONV_IMPLICIT_GEMM"), "must set MIOPEN_DEBUG_CONV_IMPLICIT_GEMM");
        assert!(content.contains("MIOPEN_FIND_MODE"), "must set MIOPEN_FIND_MODE");
        assert!(content.contains("MIOPEN_FIND_ENFORCE"), "must set MIOPEN_FIND_ENFORCE");
    }
    ...
    fn test_rocm_optimization_performance_vars() {
        ...
        assert!(content.contains("HSA_ENABLE_SDMA"), "must set HSA_ENABLE_SDMA");
        assert!(content.contains("GPU_MAX_HEAP_SIZE"), "must set GPU_MAX_HEAP_SIZE");
        assert!(content.contains("GPU_MAX_ALLOC_PERCENT"), "must set GPU_MAX_ALLOC_PERCENT");
    }
    ...
    fn test_rocm_optimization_pytorch_vars() {
        ...
        assert!(content.contains("TORCH_CUDA_ARCH_LIST"), "must set TORCH_CUDA_ARCH_LIST");
        assert!(content.contains("PYTORCH_ALLOC_CONF"), "must set PYTORCH_ALLOC_CONF");
        assert!(content.contains("PYTORCH_HIP_ALLOC_CONF"), "must set PYTORCH_HIP_ALLOC_CONF");
    }
    ...
    fn test_rocm_optimization_mpi_vars() {
        ...
        assert!(content.contains("OMPI_MCA_opal_cuda_support"), "must set OMPI MCA CUDA support");
    ...
    fn test_rocm_optimization_channel_selection() {
        let content = env_setup::generate_env_file_content(
            "0", "/opt/rocm", "6.4.3", "legacy", "gfx1030", "10.3.0",
        );
