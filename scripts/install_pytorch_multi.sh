...
if [ "$ROCM_CHANNEL" = "preview" ]; then
    ...
    case "${ROCM_VERSION%%.*}.${ROCM_VERSION#*.}" in
        ...
        7.0*|7.1*) INDEX_URL="https://download.pytorch.org/whl/nightly/rocm7.0" ; EXTRA_FLAGS=(--pre --index-url "$INDEX_URL") ;;
        7.0.2*) INDEX_URL="https://download.pytorch.org/whl/nightly/rocm7.0" ; EXTRA_FLAGS=(--pre --index-url "$INDEX_URL") ;;
        *) INDEX_URL="https://download.pytorch.org/whl/nightly/rocm7.0" ; EXTRA_FLAGS=(--pre --index-url "$INDEX_URL") ;;
    esac
fi
